...
import 'package:varadvani/presentation/providers/profile/update_profile_image_provider.dart';
...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  void initState() {
    ...
    if (currentState.data != null) {
      ...
      _profileImageUrl = _latestUser!.profileUrl;
      _initControllers(_latestUser!);
    } else {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) ref.read(getProfileProvider.notifier).getProfile();
      });
    }
  }
  ...
  Future<void> _pickImage() async {
    ...
    File finalFile = await compressImage(imageFile);
    MultipartFile file = await MultipartFile.fromFile(finalFile.path);
    ref
        .read(updateProfileImageProvider.notifier)
        .updateProfileImage(FormData.fromMap({'profileImage': file}));
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<UpdateProfileImageState>(updateProfileImageProvider, (
      previous,
      next,
    ) {
      if (next.isLoading) {
        LoaderDialog.show(
          context,
          message:
              'Updating image', //AppLocalizations.of(context)!.deleting_account,
        );
      }
      if (next.error != null) {
        if (previous?.isLoading == true) {
      ...
      if (next.data != null && previous?.isLoading == true) {
        ...
        setState(() {
          _profileImageUrl = next.data!.data.imageUrl;
        });
      }
    });

    return profileState.isLoading ||
            (profileState.data == null && profileState.error == null)
